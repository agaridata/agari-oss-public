...
#define dns_free(x) udns_free(x)
UDNS_API void
udns_free(struct dns_ctx *ctx);
...
#define dns_open(x) udns_open(x)

/* open and return UDP socket */
UDNS_API int
udns_open(struct dns_ctx *ctx);
...
struct dns_rr_ns { /* The NS RRset */
  dns_rr_common(dnsns);
  char **dnsns_ns; /* array of NSs */
};
UDNS_API dns_parse_fn dns_parse_ns; /* NS RR parsing routine */
typedef void /* NS RR callback */
dns_query_ns_fn(struct dns_ctx *ctx, struct dns_rr_ns *result, void *data);
/* submit NS IN query */
UDNS_API struct dns_query *
dns_submit_ns(struct dns_ctx *ctx, const char *name, int flags,
              dns_query_ns_fn *cbck, void *data);
/* resolve NS IN query */
UDNS_API struct dns_rr_ns *
dns_resolve_ns(struct dns_ctx *ctx, const char *name, int flags);
